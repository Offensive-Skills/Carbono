...
class ChallengesFrame(ctk.CTkFrame):
 ...
 # CARGA EL CONTENIDO DE LA PARTE SUPERIOR (FILTRADO DE RETOS).
    ...
    def setup_filters(self, frame):
        ...
        level_menu = ctk.CTkOptionMenu(frame, variable=self.level_var, values=levels,font=(Config.font_letter,15),dropdown_font=(Config.font_letter,15), fg_color=Config.verde_primario, dropdown_fg_color=Config.verde_primario, button_color=Config.verde_oscuro, button_hover_color=Config.verde_muy_oscuro)
        level_menu.grid(row=0, column=0, padx=5, pady=10, sticky="w")

        topic_menu = ctk.CTkOptionMenu(frame, variable=self.topic_var, values=topics,font=(Config.font_letter,15),dropdown_font=(Config.font_letter,15), fg_color=Config.verde_primario, dropdown_fg_color=Config.verde_primario, button_color=Config.verde_oscuro, button_hover_color=Config.verde_muy_oscuro)
        topic_menu.grid(row=0, column=1, padx=5, pady=10, sticky="w")

        mode_menu = ctk.CTkOptionMenu(frame, variable=self.mode_var, values=mode,font=(Config.font_letter,15),dropdown_font=(Config.font_letter,15), fg_color=Config.verde_primario, dropdown_fg_color=Config.verde_primario, button_color=Config.verde_oscuro, button_hover_color=Config.verde_muy_oscuro)
        mode_menu.grid(row=0, column=2, padx=5, pady=10, sticky="w")

        search_button = ctk.CTkButton(frame, text="Buscar",font=(Config.font_letter,18), command=self.load_challenges, fg_color=Config.verde_primario, hover_color=Config.verde_secundario)
 ...
 # se realiza una consulta para obtener de nuevo el listado de retos que se almacena en "self.challenges"
    ...
    def load_challenges(self):
        params = {'token': Config.api_token, 'level': self.level_var.get(), 'topic': self.topic_var.get(), 'type': self.mode_var.get()}
        ...
        if isinstance(challenges_data, list):
            ...
            self.challenges = [Challenge(v['name'], v['description'], v['is_docker'], v['version'], v['id'], v['has_file'], v['completed']) for v in challenges_data]
